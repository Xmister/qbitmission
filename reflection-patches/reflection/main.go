...
// Test variables - only used during testing to inject mock clients
var (
	testMode = false
...
// SetTestMode enables test mode and sets the test client for dependency injection
func SetTestMode(client *http.Client, useSync bool) {
	testMode = true
	testClient = client
	testUseSync = useSync
}
...
// Removed global qBTConn to fix race condition with multiple concurrent connections
...
func parseIDsField(args *json.RawMessage, conn *qBT.Connection) qBT.TorrentInfoList {
	conn.UpdateTorrentsList()

	if args == nil {
		log.Debug("No IDs provided")
		return conn.TorrentsList.Slice()
	}
	...
	case float64:
		...
		return []*qBT.TorrentInfo{conn.TorrentsList.ByID(qBT.ID(ids))}
	case []interface{}:
		...
		for i, value := range ids {
			...
			case float64:
				result[i] = conn.TorrentsList.ByID(qBT.ID(id))
			case string:
				hash := qBT.Hash(id)
				result[i] = conn.TorrentsList.ByHash(hash)
			}
			if result[i] == nil {
	...
	case string:
		...
		if *useSync {
			return conn.TorrentsList.GetActive()
		} else {
			return conn.TorrentsList.Slice()
		}
	default:
...
func parseActionArgument(args json.RawMessage, conn *qBT.Connection) qBT.TorrentInfoList {
	var req struct {
		Ids json.RawMessage
	}
	...
	return parseIDsField(&req.Ids, conn)
}
...
func MapPropsPeers(dst JsonMap, hash qBT.Hash, conn *qBT.Connection) {
	url := conn.MakeRequestURLWithParam("sync/torrentPeers", map[string]string{"hash": string(hash), "rid": "0"})
	torrents := conn.DoGET(url)
...
func TorrentGet(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	var req transmission.GetRequest
	err := json.Unmarshal(args, &req)
	Check(err)

	torrents := parseIDsField(req.Ids, conn)
	...
	for i, torrentItem := range torrents {
		...
		if propsGeneralNeeded {
			...
			propsCache.GetOrFill(hash, translated, severalIDsRequired, func(dest JsonMap) {
				propGeneral := conn.GetPropsGeneral(hash)
		...
		if trackersNeeded || trackerStatsNeeded {
			...
			trackersCache.GetOrFill(hash, translated, severalIDsRequired, func(dest JsonMap) {
				trackers := conn.GetPropsTrackers(hash)
	...
	addRemovedList(req.Ids, response, conn)
	return response, "success"
}

func addRemovedList(idsField *json.RawMessage, resp JsonMap, conn *qBT.Connection) {
	if idsField == nil {
		return
	}
	var ids interface{}
	...
	switch ids.(type) {
	case string:
		resp["removed"] = conn.TorrentsList.GetRemoved()
	}
}
...
func SessionGet(conn *qBT.Connection) (JsonMap, string) {
	session := make(JsonMap)
	for key, value := range transmission.SessionGetBase {
		session[key] = value
	}

	prefs := conn.GetPreferences()
...
func SessionStats(conn *qBT.Connection) (JsonMap, string) {
	session := make(JsonMap)
	for key, value := range transmission.SessionStatsTemplate {
		session[key] = value
	}

	torrentList := conn.TorrentsList.AllItems()
...
func TorrentPause(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	torrents := parseActionArgument(args, conn)
	log.WithField("hashes", torrents.Hashes()).Debug("Stopping torrents")
	conn.PostWithHashes("torrents/pause", torrents)
	return JsonMap{}, "success"
}

func TorrentResume(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	torrents := parseActionArgument(args, conn)
	log.WithField("hashes", torrents.Hashes()).Debug("Starting torrents")

	conn.PostWithHashes("torrents/resume", torrents)
	return JsonMap{}, "success"
}

func TorrentRecheck(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	torrents := parseActionArgument(args, conn)
	log.WithField("hashes", torrents.Hashes()).Debug("Verifying torrents")

	conn.PostWithHashes("torrents/recheck", torrents)
	return JsonMap{}, "success"
}

func TorrentDelete(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	var req struct {
		Ids             json.RawMessage
	...
	url := conn.MakeRequestURLWithParam("torrents/delete", params)
	conn.DoGET(url)
...
func UploadTorrent(metainfo *[]byte, urls *string, req *transmission.TorrentAddRequest, paused bool, conn *qBT.Connection) {
	...
	conn.DoPOST(conn.MakeRequestURL("torrents/add"), mime.FormDataContentType(), &buffer)
...
func TorrentAdd(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	...
	if torrent := conn.TorrentsList.ByHash(newHash); torrent != nil {
		return JsonMap{
	...
	for retries := 0; retries < 100; retries++ {
		...
		conn.UpdateTorrentsList()
		torrent = conn.TorrentsList.ByHash(newHash)
		if torrent != nil {
...
func TorrentSet(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	var req struct {
		Ids            *json.RawMessage
	...
	if req.Files_wanted != nil || req.Files_unwanted != nil {
		...
		for priority, fileIds := range priorityGroups {
			...
			response := conn.PostForm(conn.MakeRequestURL("torrents/filePrio"), params)
			log.WithFields(log.Fields{
				"hash":     string(torrent.Hash),
				"fileIds":  fileIds,
				"priority": priority,
...
func TorrentSetLocation(args json.RawMessage, conn *qBT.Connection) (JsonMap, string) {
	var req struct {
		Ids      *json.RawMessage
	...
	if extraArgs.firstLastPiecesFirst != ARGUMENT_NOT_SET {
		for _, torrent := range torrents {
			conn.SetFirstLastPieceFirst(torrent.Hash, extraArgs.firstLastPiecesFirst == ARGUMENT_TRUE)
		}
	}

	if extraArgs.sequentialDownload != ARGUMENT_NOT_SET {
		for _, torrent := range torrents {
			conn.SetSequentialDownload(torrent.Hash, extraArgs.sequentialDownload == ARGUMENT_TRUE)
		}
	}
	...
	conn.PostForm(conn.MakeRequestURL("torrents/setLocation"), params)

	return JsonMap{}, "success"
}

func handler(w http.ResponseWriter, r *http.Request) {
	// Create a new connection for each request to avoid race conditions
	var conn qBT.Connection
	var cl *http.Client
	var useSyncMode bool
	
	if testMode && testClient != nil {
		// Use injected test client for testing
		cl = testClient
		useSyncMode = testUseSync
	} else if *disableKeepAlive {
		tr := &http.Transport{
	...
	conn.Init(*apiAddr, cl, useSyncMode)
	...
	if !conn.IsLoggedIn() {
		...
		if present {
			authOK = conn.Login(username, password)
		} else {
			authOK = conn.Login("", "")
		}
		if !authOK {
	...
	switch req.Method {
	case "session-get":
		resp, result = SessionGet(&conn)
	case "free-space":
		resp, result = FreeSpace(req.Arguments)
	case "torrent-get":
		resp, result = TorrentGet(req.Arguments, &conn)
	case "session-stats":
		resp, result = SessionStats(&conn)
	case "torrent-stop":
		resp, result = TorrentPause(req.Arguments, &conn)
	case "torrent-start":
		resp, result = TorrentResume(req.Arguments, &conn)
	case "torrent-start-now":
		resp, result = TorrentResume(req.Arguments, &conn)
	case "torrent-verify":
		resp, result = TorrentRecheck(req.Arguments, &conn)
	case "torrent-remove":
		resp, result = TorrentDelete(req.Arguments, &conn)
	case "torrent-add":
		resp, result = TorrentAdd(req.Arguments, &conn)
	case "torrent-set":
		resp, result = TorrentSet(req.Arguments, &conn)
	case "torrent-set-location":
		resp, result = TorrentSetLocation(req.Arguments, &conn)
	default:
